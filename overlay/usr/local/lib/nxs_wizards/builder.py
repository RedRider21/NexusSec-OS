...
_PROFILES = [("__none__", None), ("Pen Testing", "pentest"),
...
class _FieldRow:
    def __init__(self, builder, data=None):
        ...
        self.key = Gtk.Entry(); self.key.set_placeholder_text(_t("wb.f_key_ph"))
        self.key.set_text(data.get("key", ""))
        self.type = Gtk.ComboBoxText()
        self.type.append("text", _t("wb.type_text")); self.type.append("file", _t("wb.type_file"))
        self.type.set_active_id(data.get("type", "text"))
        self.req = Gtk.CheckButton.new_with_label(_t("wb.required"))
        ...
        top.pack_start(Gtk.Label(label=_t("wb.l_key")), False, False, 0)
        ...
        self.label.set_placeholder_text(_t("wb.f_label_ph"))
        self.label.set_text(data.get("label", ""))
        self.ph = Gtk.Entry()
        self.ph.set_placeholder_text(_t("wb.f_ph_ph"))
...
class _ModeRow:
    def __init__(self, builder, data=None):
        ...
        top = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        self.label = Gtk.Entry()
        self.label.set_placeholder_text(_t("wb.m_label_ph"))
        self.label.set_text(data.get("label", ""))
        self.default = Gtk.CheckButton.new_with_label(_t("wb.mode_default"))
        self.default.set_active(bool(data.get("default")))
        rm = common.icon_button("", "list-remove-symbolic")
        rm.connect("clicked", lambda _b: builder._remove(builder.mode_rows, self))
        top.pack_start(Gtk.Label(label=_t("wb.l_mode")), False, False, 0)
        ...
        self.desc = Gtk.Entry()
        self.desc.set_placeholder_text(_t("wb.desc_short_ph"))
        self.desc.set_text(data.get("desc", ""))
        box.pack_start(self.desc, False, False, 0)

        self.vars = Gtk.Entry()
        self.vars.set_placeholder_text(_t("wb.m_vars_ph"))
        vv = data.get("vars") or {}
...
class _OptionRow:
    def __init__(self, builder, data=None):
        ...
        top = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        self.label = Gtk.Entry()
        self.label.set_placeholder_text(_t("wb.o_label_ph"))
        self.label.set_text(data.get("label", ""))
        self.default = Gtk.CheckButton.new_with_label(_t("wb.opt_default"))
        self.default.set_active(bool(data.get("default")))
        rm = common.icon_button("", "list-remove-symbolic")
        rm.connect("clicked", lambda _b: builder._remove(builder.option_rows, self))
        top.pack_start(Gtk.Label(label=_t("wb.l_option")), False, False, 0)
        top.pack_start(self.label, True, True, 0)
        top.pack_start(self.default, False, False, 0)
        top.pack_end(rm, False, False, 0)
        box.pack_start(top, False, False, 0)
...
class _StepRow:
    def __init__(self, builder, data=None):
        ...
        top.pack_start(Gtk.Label(label=_t("wb.tool")), False, False, 0)
        ...
        self.desc = Gtk.Entry()
        self.desc.set_placeholder_text(_t("wb.s_desc_ph"))
        self.desc.set_text(data.get("desc", ""))
        box.pack_start(self.desc, False, False, 0)

        self.args = Gtk.Entry()
        self.args.set_placeholder_text(_t("wb.s_args_ph"))
        ...
        self.modes.set_placeholder_text(_t("wb.s_modes_ph"))
        self.modes.set_text(", ".join(data.get("modes", []) or []))
        self.needs = Gtk.Entry()
        self.needs.set_placeholder_text(_t("wb.s_needs_ph"))
        ...
        for k, lab in (("tor", _t("wb.st_tor")), ("raw", _t("wb.st_raw")),
                       ("container", _t("wb.st_container")), ("local", _t("wb.st_local"))):
            self.anon.append(k, lab)
        self.anon.set_active_id(s.get("anon", "tor"))
        self.sonly = Gtk.ComboBoxText()
        for k, lab in (("", _t("wb.st_always")), ("on", _t("wb.st_on")),
                       ("off", _t("wb.st_off"))):
            self.sonly.append(k, lab)
        self.sonly.set_active_id(s.get("only", "") or "")
        self.sskip = Gtk.CheckButton.new_with_label(_t("wb.st_skip"))
        self.sskip.set_active(bool(s.get("skip")))
        self.sflags = Gtk.Entry()
        self.sflags.set_placeholder_text(_t("wb.st_flags_ph"))
        self.sflags.set_text(s.get("flags", ""))
        srow.pack_start(Gtk.Label(label=_t("wb.stealth_lbl")), False, False, 0)
        ...
        self.pvar = Gtk.Entry()
        self.pvar.set_placeholder_text(_t("wb.p_var_ph"))
        self.pvar.set_text(pr.get("var", ""))
        self.pextract = Gtk.ComboBoxText()
        for k, lab in (("lines", _t("wb.ex_lines")), ("nmap_up", _t("wb.ex_nmap")),
                       ("subdomains", _t("wb.ex_sub")), ("urls", _t("wb.ex_urls")),
                       ("regex", _t("wb.ex_regex"))):
        ...
        self.pfrom.set_placeholder_text(_t("wb.p_from_ph"))
        self.pfrom.set_text(pr.get("from", "stdout"))
        self.pregex = Gtk.Entry()
        self.pregex.set_placeholder_text(_t("wb.p_regex_ph"))
        if ex.startswith("regex:"):
            self.pregex.set_text(ex[6:])
        prow.pack_start(Gtk.Label(label=_t("wb.produce_lbl")), False, False, 0)
...
class BuilderScreen:
    ...
    def _section(self, parent, text, adder=None):
        ...
        if adder:
            add = common.icon_button(_t("wb.add"), "list-add-symbolic")
    ...
    def show(self):
        ...
        head = Gtk.Label(label=(_t("wb.head_edit") if self.edit_id
                                else _t("wb.head_new")))
        ...
        self.name.set_placeholder_text(_t("wb.name_ph"))
        self.name.set_text(p.get("name", ""))
        self.desc = Gtk.Entry(); self.desc.set_hexpand(True)
        self.desc.set_placeholder_text(_t("wb.desc_ph"))
        self.desc.set_text(p.get("description", ""))
        self.icon = Gtk.Entry()
        self.icon.set_placeholder_text(_t("wb.icon_ph"))
        self.icon.set_text(p.get("icon", "system-run-symbolic"))
        self.profile = Gtk.ComboBoxText()
        for lab, val in _PROFILES:
            self.profile.append(val or "", _t("wb.prof_none") if lab == "__none__" else lab)
        self.profile.set_active_id(p.get("profile") or "")
        grid.attach(Gtk.Label(label=_t("wb.l_name"), xalign=0), 0, 0, 1, 1)
        grid.attach(self.name, 1, 0, 1, 1)
        grid.attach(Gtk.Label(label=_t("wb.l_desc"), xalign=0), 0, 1, 1, 1)
        grid.attach(self.desc, 1, 1, 1, 1)
        grid.attach(Gtk.Label(label=_t("wb.l_icon"), xalign=0), 0, 2, 1, 1)
        grid.attach(self.icon, 1, 2, 1, 1)
        grid.attach(Gtk.Label(label=_t("wb.l_profile"), xalign=0), 0, 3, 1, 1)
        grid.attach(self.profile, 1, 3, 1, 1)
        form.pack_start(grid, False, False, 0)

        self.stealthdef = Gtk.CheckButton.new_with_label(_t("wb.stealth_def"))
        ...
        self._section(form, _t("wb.sec_fields"),
        ...
        self._section(form, _t("wb.sec_modes"),
                      adder=lambda: self._add(self.mode_rows, _ModeRow, cont_m))
        std = Gtk.Button(label=_t("wb.add_std"))
        ...
        self._section(form, _t("wb.sec_options"),
        ...
        self._section(form, _t("wb.sec_steps"),
        ...
        if not self.prefill:
            ...
            self._add(self.field_rows, _FieldRow, cont_f,
                      {"key": "target", "label": _t("wb.f_target"), "required": True})

        # --- barra pulsanti (fuori dallo scroll) ---
        bar = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        cancel = common.icon_button(_t("wiz.ui.cancel"), "go-previous-symbolic")
        cancel.connect("clicked", lambda _b: self.on_done())
        save = common.icon_button(_t("wb.save"), "document-save-symbolic",
                                  primary=True)
    ...
    def _add_standard_modes(self):
        ...
        presets = [
            {"label": _t("wb.std_light"), "desc": _t("wb.std_light_d"),
             "default": True, "vars": {}},
            {"label": _t("wb.std_deep"), "desc": _t("wb.std_deep_d"),
             "vars": {}},
            {"label": _t("wb.std_stealth"), "desc": _t("wb.std_stealth_d"), "vars": {}},
        ]
        for pr in presets:
    ...
    # ---- salvataggio ----
    def _error(self, msg):
        common.info_dialog(_t("wb.invalid"), msg, level="error", parent=self.win)

    def _save(self):
        name = self.name.get_text().strip()
        if not name:
            self._error(_t("wb.err_name"))
            return
        ...
        if recipes.save_custom(wid, wiz):
            common.info_dialog(
                _t("wb.saved"),
                _t("wb.saved_body") % name,
                parent=self.win)
            self.on_done()
        else:
            self._error(_t("wb.err_write"))
