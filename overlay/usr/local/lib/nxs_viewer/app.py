...
"""Visualizzatore di immagini di NexusSec - GTK3 + GdkPixbuf.
...
def _chiaro() -> bool:
    try:
        from nxs_cc import palette  # assente: ripiego scuro
...
class Visualizzatore(Gtk.Window):
    ...
    def _stato(self):
        ...
        self.lbl_info.get_style_context().add_class("nxs-val")
    ...
    def come_sfondo(self):
        ...
        if not p:
            return
        try:
            subprocess.Popen(["nxs-wallpaper", "set", p],
    ...
    def apri_cartella(self):
        ...
        if p:
            try:
                subprocess.Popen(["pcmanfm", os.path.dirname(p)],
                                 stdout=subprocess.DEVNULL,
                                 stderr=subprocess.DEVNULL)
            except OSError:
                pass
    ...
    def proprieta(self):
        ...
        for r, (k, v) in enumerate(righe):
            ...
            a.get_style_context().add_class("nxs-key")
