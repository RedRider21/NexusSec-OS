...
"""Editor di testo di NexusSec - GTK3 + GtkSourceView 4.
...
Impostazioni in ~/.config/nxs/editor.json.
"""
import json
import os
import sys
...
# Percorsi NexusSec (niente modulo "paths": costanti dirette).
CONF_DIR = os.path.join(os.environ.get("XDG_CONFIG_HOME")
                        or os.path.expanduser("~/.config"), "nxs")
DATA_STYLES = "/usr/local/share/nexussec/sourceview-styles"
CACHE_STYLES = os.path.join(os.environ.get("XDG_CACHE_HOME")
                            or os.path.expanduser("~/.cache"),
                            "nxs", "sourceview-styles")
...
try:
    from nxs_i18n import t as _t, taccel as _ta
except Exception:                                   # noqa: BLE001
...
# ---------------------------------------------------------------------------
def _conf_file():
    return os.path.join(CONF_DIR, "editor.json")
...
def _accento() -> str:
    """Accent del preset attivo, con ripiego sul ciano di NexusSec."""
    try:
...
def _chiaro() -> bool:
    # NexusSec ha un ambiente scuro fisso: l'editor usa lo schema scuro.
    return False


def _schema_colore(chiaro: bool):
    """Prepara (e restituisce) lo schema GtkSourceView di NexusSec.
    ...
    """
    nome = "nxs-light" if chiaro else "nxs-dark"
    mgr = GtkSource.StyleSchemeManager.get_default()
    modello = os.path.join(DATA_STYLES, nome + ".xml")
    if not os.path.isfile(modello):
        modello = None
...
class Documento(Gtk.Box):
    ...
    def __init__(self, editor, percorso: str | None = None):
        ...
        self.tab.get_style_context().add_class("nxs-editor-tab")
    ...
    def _su_modifica(self, _buf):
        ...
        (ctx.add_class if mod else ctx.remove_class)("nxs-editor-modificato")
...
class Editor(Gtk.Window):
    ...
    def _barra_strumenti(self):
        ...
        barra.get_style_context().add_class("nxs-editor-barra")
    ...
    def _voce(self, menu, etichetta, callback, scorciatoia=""):
        ...
        if scorciatoia:
            ...
            acc.get_style_context().add_class("nxs-val")
    ...
    def _barra_ricerca(self):
        ...
        box.get_style_context().add_class("nxs-editor-trova")
        self.box_trova = box
        ...
        self.lbl_occorrenze.get_style_context().add_class("nxs-val")
    ...
    def mostra_scorciatoie(self):
        ...
        for titolo, voci in self._elenco_scorciatoie():
            ...
            for _k, _m, testo, desc, _cb in voci:
                ...
                a.get_style_context().add_class("nxs-val")
    ...
    def _shortcuts_window(self):
        ...
        sezione = Gtk.ShortcutsSection(section_name="nxs-editor",
